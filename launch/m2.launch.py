...
def generate_launch_description() -> LaunchDescription:
    """
    Build the launch description for the m2 robot model.
    """
    robot_model = 'm2'
    ...
    ldes: list[LaunchDescriptionEntity] = [
        ...
        DeclareLaunchArgument(
            'params_file',
            default_value=os.path.join(get_package_share_directory('robot_agr_4sw'), 'config', 'example_m2.yaml'),
            description='Path to params file. If empty, the selected model launch picks its default.',
        ),
        DeclareLaunchArgument(
            'bridge_file',
            default_value=os.path.join(
                get_package_share_directory('robot_agr_4sw'), 'config', 'example_m2_bridge.yaml'
            ),
    ...
    ldes.extend(
        [
            ...
            DeclareLaunchArgument(
                'four_swerve_kinematics_node_topic_remappings', default_value='', description=rlh.TOPIC_REMAPPINGS_DESC
            ),
            ####################################################################
            # NODE OPTIONS
            ####################################################################
